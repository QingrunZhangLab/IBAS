...
option_list <- list(
  ...
  make_option(c("--cisBP"), action = "store", type = "numeric", 
              default = "1000000",
              help = "Number of cis-bps to consider. Default value is %default."),
...
for(comp in file_list){
  if(!dir.exists(paste0(args$temp_folder, "/", d, "/", comp))){
    dir.create(paste0(args$temp_folder, "/", d, "/", comp))
  }
}
...
for(n in nums){
  ...
  if(nrow(curr_results)==0){
    print("Insufficient results for component", n, " of ", d, " to generate SKAT files...")
    next
  }
  ...
  fwrite(curr_SNP_Weights, paste0(args$temp_folder, "/", d, "/V", n, "/Snp.Weights.txt"), col.names = FALSE, sep = "\t")
  ...
  SnpSets <- fread(paste0(args$temp_folder, "/", d, "/V", n, "/Snp.Sets.txt"), header = FALSE)
  obj.SNPWeight <- Read_SNP_WeightFile(paste0(args$temp_folder, "/", d, "/V", n, "/Snp.Weights.txt"))
...
if(!dir.exists(dirname(args$results_file))){
  dir.create(dirname(args$results_file))
}

fwrite(all_results, paste0(args$results_file, "_", d, ".csv"))